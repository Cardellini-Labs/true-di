...
const itemsOf = <C extends object>(container: C): Map<keyof C, any> =>
  assertExists((container as any)[$INSTANCES], 'Argument is not a container');

export const isReady = <C extends object>(
  container: C,
  name: keyof C,
): boolean => itemsOf(container).has(name);
...
export const factoriesFrom = <C extends object, N extends keyof C = keyof C>(
  ...
  mapObject<C, N, IPureFactories<Pick<C, N>>>(
    container,
    (name) => () => container[name],
    names,
  );
...
const diContainer: {
  ...
  const container: Private & Public = allNames(factories).reduce<
    Private & Public
  >(
    (containerObj, name) =>
      Object.defineProperty(containerObj, name, {
        configurable: false,
        enumerable: Object.getOwnPropertyDescriptor(factories, name)!
          .enumerable,
  ...
  return publicFactories != null
    ? narrowObject(container, allNames(publicFactories))
    : container;
};
